        ...
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-5 duration-700">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b">
                    <div className="flex items-center gap-6">
                        <div className="w-20 h-20 rounded-3xl bg-primary/10 flex items-center justify-center text-primary font-bold text-3xl">
                            {group.avatarUrl ? (
                                <img src={group.avatarUrl} alt={group.name} className="w-full h-full object-cover rounded-3xl" />
                            ) : (
                        ...
                        <div className="space-y-1">
                            ...
                            <p className="text-muted-foreground text-lg">{group.description || t("group.defaultDescription")}</p>

                            {group.storageLimitBytes && (
                                <div className="flex items-center gap-4 pt-2">
                                    <div className="flex-1 w-[240px] space-y-1.5">
                    ...
                    <div className="flex flex-col items-end gap-3">
                        <div className="flex items-center gap-3">
                            {!isViewer && (
                                <Button
                                    ...
                                    size="lg"
                                    className="rounded-xl gap-2 h-12 shadow-sm transition-all"
                            ...
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <Button variant="outline" size="icon" className="w-12 h-12 rounded-xl border-muted-foreground/20">
                            ...
                            {!isViewer && (
                                <Link to={`/groups/${groupId}/albums/new`}>
                                    <Button size="lg" className="rounded-xl h-12 px-6 gap-2 shadow-lg">
                        ...
                        {!isViewer && showInviteCode && (
                            <div className="flex flex-col gap-2">
                                <div className="flex items-center gap-3 p-3 bg-primary/5 border border-primary/20 rounded-2xl animate-in slide-in-from-top-2 duration-300">
                                    <div className="text-xs font-bold uppercase tracking-widest text-primary/60 px-2 min-w-[130px]">{t("group.memberInvite")}</div>
                                    <code className="text-xl font-black tracking-tighter text-primary bg-white px-4 py-1 rounded-lg border shadow-sm flex-1 text-center">
                                        {group.inviteCode}
                                    </code>
                                    <Button variant="ghost" size="icon" onClick={() => handleCopyCode(group.inviteCode, false)} className="text-primary hover:bg-primary/10 rounded-xl">
                                        {copied ? <CheckCircle2 className="w-5 h-5" /> : <Copy className="w-5 h-5" />}
                                    </Button>
                                </div>
                                <div className="flex items-center gap-3 p-3 bg-secondary/20 border border-secondary/30 rounded-2xl animate-in slide-in-from-top-2 duration-300">
                                    <div className="text-xs font-bold uppercase tracking-widest text-secondary-foreground/60 px-2 min-w-[130px]">{t("group.guestInvite")}</div>
                                    <code className="text-xl font-black tracking-tighter text-secondary-foreground bg-white px-4 py-1 rounded-lg border shadow-sm flex-1 text-center">
                                        {group.guestInviteCode}
                                    </code>
                                    <Button variant="ghost" size="icon" onClick={() => handleCopyCode(group.guestInviteCode, true)} className="text-secondary-foreground hover:bg-secondary/20 rounded-xl">
                                        {guestCopied ? <CheckCircle2 className="w-5 h-5" /> : <Copy className="w-5 h-5" />}
                                    </Button>
                                </div>
                            </div>
                        )}
            ...
            <div className="flex gap-8 border-b">
                <button
                    ...
                    className={`pb-4 text-lg font-bold transition-all relative ${activeTab === "albums" ? 'text-primary' : 'text-muted-foreground hover:text-foreground'}`}
                >
                    {t("group.tabs.albums")}
                ...
                <button
                    ...
                    className={`pb-4 text-lg font-bold transition-all relative ${activeTab === "members" ? 'text-primary' : 'text-muted-foreground hover:text-foreground'}`}
                >
            ...
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
                {activeTab === "albums" ? (
                    <div className="space-y-6">
                        ...
                        {albums.length === 0 ? (
                            <div className="flex flex-col items-center justify-center p-20 border-2 border-dashed rounded-[2.5rem] bg-muted/20 text-center space-y-6">
                                <div className="w-20 h-20 bg-primary/10 rounded-full flex items-center justify-center text-primary">
                                    <ImageIcon className="w-10 h-10" />
                                </div>
                                <div className="space-y-2">
                                    <h3 className="text-2xl font-bold">{t("group.albumsEmptyTitle")}</h3>
                                ...
                                {!isViewer && (
                                    <Link to={`/groups/${groupId}/albums/new`}>
                                        <Button size="lg" className="rounded-xl px-8 shadow-md">{t("group.albumsEmptyCta")}</Button>
                                    </Link>
                                )}
                            </div>
                        ) : (
        ...
        <>
            ...
            <div className="flex items-center gap-4 p-5 bg-card border rounded-3xl hover:border-primary/30 transition-all shadow-sm group">
                ...
                <div className="flex flex-col items-end gap-2">
                    ...
                    {canRemove && (
                        <Button variant="ghost" size="sm" onClick={handleRemove} className="opacity-0 group-hover:opacity-100 h-7 text-xs text-destructive hover:bg-destructive/10 hover:text-destructive transition-all px-2">
        ...
        <>
            ...
            >
                ...
                {userRole === "OWNER" && (
                    <div className="absolute top-6 right-6 z-20 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button
                            ...
                            className="w-12 h-12 rounded-full bg-black/20 hover:bg-destructive hover:text-white backdrop-blur-md text-white border border-white/10"
                        >
                ...
                <div className="absolute inset-0 p-8 flex flex-col justify-end gap-3 translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                    <div className="space-y-1">
                        <div className="flex items-center gap-2 text-primary-foreground/70 text-xs font-bold uppercase tracking-widest">
                            <ImageIcon className="w-3.5 h-3.5" />
                            {t("album.collection")}
                        </div>
                        <h3 className="text-3xl font-bold text-white tracking-tight">{album.name}</h3>
                    </div>

                    <p className="text-white/70 line-clamp-2 text-sm leading-relaxed mb-4 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
                        {album.description || "A beautiful collection of memories preserved forever."}
                    </p>

                    <div className="flex items-center justify-between pt-4 border-t border-white/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-200">
